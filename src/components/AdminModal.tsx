...
export function AdminModal({ isOpen, onClose, onSave, initialData }: AdminModalProps) {
  ...
  const handleAnalyzeBlog = async () => {
    if (!formData.blogUrl) return;
    setAnalyzing(true);
    try {
      // Use our server proxy to fetch content
      const response = await fetch(`/api/fetch-blog?url=${encodeURIComponent(formData.blogUrl)}`);
      const data = await response.json();
      
      if (data.content) {
        const suggested = await suggestTags(data.content);
        // Merge with existing tags, filter duplicates
        const mergedTags = Array.from(new Set([...(formData.tags || []), ...suggested]));
        setFormData({ ...formData, tags: mergedTags });
      }
    } catch (error) {
      console.error("Failed to analyze blog:", error);
    } finally {
      setAnalyzing(false);
    }
  };
            ...
            <label className="text-xs font-black uppercase tracking-widest text-slate-400 ml-1 flex items-center justify-between">
              ...
              {formData.blogUrl && (
                <button
                  ...
                  disabled={analyzing}
                  className="text-[10px] flex items-center gap-1.5 text-dati-purple hover:text-dati-blue font-black tracking-wider uppercase px-2 py-1 rounded bg-dati-purple/5 transition-colors"
                >
                  {analyzing ? <Loader2 size={10} className="animate-spin" /> : <Sparkles size={10} />}
                  {analyzing ? "Analisando..." : "Sugerir Tags IA"}
                </button>
              )}
            </label>
            <input
